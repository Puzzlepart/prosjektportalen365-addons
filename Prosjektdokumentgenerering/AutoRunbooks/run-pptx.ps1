                ...
                while ($KeepSearching) {
                    ...
                    foreach ($Para in $AllParagraphs) {
                        ...
                        if ($ParaText -match [regex]::Escape($Key)) {
                            ...
                            $TextMatches = [regex]::Matches($Para.Value, '<a:t>([^<]*)</a:t>')
                            ...
                            $FirstRunMatch = [regex]::Match($Para.Value, '<a:r>(?<props><a:rPr.*?</a:rPr>)?<a:t>')
                            ...
                            $BeforeRuns = $Para.Value -replace '(<a:r>.*$)', ''
                            # Get the paragraph part after last </a:r>
                            $AfterRuns = $Para.Value -replace '(^.*</a:r>)', ''
