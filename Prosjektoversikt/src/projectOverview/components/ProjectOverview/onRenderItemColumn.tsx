      ...
      return (
        <span>
          <TooltipHost
            ...
            content={
              <div className="ms-TooltipHost-inner">
                <h1 className="title">{item.title}</h1>
                {allColumns.map((column) => {
                  return (
                    <>
                      {column.Title.toLowerCase().includes("(text)") ? (
                        <h4>{column.Title.replace("(text)", "")}</h4>
                      ) : column.Title.toLowerCase().includes("(tekst)") ? (
                        <h4>{column.Title.replace("(tekst)", "")}</h4>
                      ) : (
                        <h4>{column.Title}</h4>
                      )}
                      {renderToolTipField(
                        column,
                        item.hoverData[column.InternalName]
                      )}
                    </>
                  );
                })}
          ...
          >
            <Link href={item.siteUrl} target="_blank">
